...
-- * Entry point
main = do
  xmproc <- spawnPipe "xmobar /home/leo/.xmobarrc" -- Start xmobar with config
...
-- * ManageHook
myManageHook = (className =? "krunner" --> doIgnore >> doFloat) <+> namedScratchpadManageHook scratchpads <+> manageDocks <+> manageHook def
-- * Layouts
-- myTabbed = tabbed shrinkText defaultTheme {
          -- fontName = "xft:Lat2-Terminus16:size=12" -- choose an installed font
...
myKeymap = ([ ("M4-/", switchProjectPrompt def)
            ...
            , ("M4-c", namedScratchpadAction scratchpads "cmus")]
               ++ map (\x -> ("M4-S-" ++ show x, shiftToProjectNr x)) [0..9] -- Move active window to the project at position x
               ++ map (\x -> ("M4-C-" ++ show x, switchActiveProjectNr x)) [0..9] -- Assign a project to position x
               ++ map (\x-> ("M4-" ++ show x, goToProjectNr x)) [0..9]) -- Go to the project at position x
-- * Scratchpads
